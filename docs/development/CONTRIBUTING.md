...
This project uses comprehensive pre-commit hooks for code quality and security. Follow these steps:
...
#### 1. Quick Tests (Safe - No Deployment)

```bash
# Run all safe tests (recommended for development)
make test-safe

# Individual test types
make test-lint      # Formatting and linting
make test-validate  # Terraform validation
...
run "test_architecture_detection" {
  ...
  assert {
    ...
    error_message = "Architecture detection failed for ARM64"
  }
...
run "test_raspberry_pi_cluster" {
  ...
  assert {
    condition = length([
      ...
      service if service.cpu_arch == "arm64"
    ]) > 0
    error_message = "No ARM64 services configured"
  }
}
```
...
```bash
# Always run safe tests before committing
make test-safe
...
- ✅ **Architecture Detection**: ARM64/AMD64 auto-detection
- ✅ **Storage Configuration**: NFS, HostPath, cloud storage
- ✅ **Service Enablement**: All service combinations
- ✅ **Resource Limits**: Architecture-specific limits
- ✅ **Mixed Clusters**: Multi-architecture deployments
- ✅ **Security Policies**: RBAC and network policies
- ✅ **Helm Configuration**: Values template generation
- ✅ **Variable Validation**: Input validation logic
