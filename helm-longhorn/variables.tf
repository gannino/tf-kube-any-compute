...
variable "chart_version" {
  ...
  default     = "1.11.0" # Updated from 1.7.2
}
...
variable "helm_disable_webhooks" {
  description = "Disable webhooks for Helm release (required to prevent uninstall blocking by admission webhooks)"
  type        = bool
  default     = true
}
...
# Simple backup configuration (for basic NFS/S3 setups)
variable "backup_target" {
  description = "Longhorn backup target URL (e.g., 'nfs://server:/path' or 's3://bucket@region/')"
  type        = string
  default     = ""
}
