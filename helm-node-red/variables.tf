# ============================================================================
# HELM-NODE-RED MODULE VARIABLES - IOT AND AUTOMATION PLATFORM
# ============================================================================
...
variable "name" {
  description = "Helm release name for Node-RED"
  type        = string
  default     = "node-red"
...
variable "chart_repo" {
  ...
  type        = string
  default     = "https://schwarzit.github.io/node-red-chart/"
...
variable "enable_persistence" {
  description = "Enable persistent storage for Node-RED data"
  type        = bool
  default     = true
}
...
variable "persistent_disk_size" {
  description = "Size of persistent disk for Node-RED data"
  type        = string
  default     = "2Gi"
...
variable "cpu_limit" {
  description = "CPU limit for Node-RED containers"
  type        = string
  default     = "500m"
...
variable "memory_limit" {
  description = "Memory limit for Node-RED containers"
  type        = string
  default     = "512Mi"

  validation {
    condition     = can(regex("^[0-9]+[KMGT]i?$", var.memory_limit))
    error_message = "Memory limit must be in format like '512Mi', '1Gi', etc."
  }
}

variable "cpu_request" {
  description = "CPU request for Node-RED containers"
  type        = string
  default     = "250m"
...
variable "memory_request" {
  description = "Memory request for Node-RED containers"
  type        = string
  default     = "256Mi"

  validation {
    condition     = can(regex("^[0-9]+[KMGT]i?$", var.memory_request))
    error_message = "Memory request must be in format like '256Mi', '1Gi', etc."
  }
}
...
variable "palette_packages" {
  description = "List of Node-RED palette packages to install during deployment"
  type        = list(string)
  default = [
