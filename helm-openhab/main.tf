...
resource "helm_release" "this" {
  ...
  values = [
    templatefile("${path.module}/templates/openhab-values.yaml.tpl", local.template_values)
  ]
...
# Wait for openHAB deployment to be ready
resource "null_resource" "wait_for_deployment" {
  depends_on = [helm_release.this]

  provisioner "local-exec" {
    command     = <<EOT
      echo "Waiting for openHAB deployment to be ready..."
